 ...
 */
function buildAuthorizeUrl({ baseUrl, clientId, redirectUri, scope, state, codeChallenge }) {
 ...
 */
async function exchangeCodeForToken({ baseUrl, clientId, clientSecret, redirectUri, code, codeVerifier }) {
  if (!fetchFn) {
    throw new Error(
  ...
  // Salesforce si aspetta application/x-www-form-urlencoded
  ...
  return data; // access_token, instance_url, id, token_type, issued_at, signature, (refresh_token se scope lo consente)
}
